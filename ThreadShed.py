...
#------------------------------------------------
# Start coding below!
# Replace artifact and split transactions
if daily_sales:
...
for trans in daily_transactions_split:
  """new_list = []
  for point in trans:
    new_list.append(point.replace("\n","").strip(" "))
  transactions_clean.append(new_list)"""
  transactions_clean = [[point.replace("\n","").strip() for point in trans] for trans in daily_transactions_split]
...
# Split thread colors and count occurrences
thread_sold_split = []
...
for color in colors:
  sentence = "Thred Shed sold {0} threads of {1} thread today".format(color_count(color), color)
  print(sentence)
